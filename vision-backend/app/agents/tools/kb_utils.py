...
def compute_missing_fields(agent: AgentState, rule: Dict) -> None:
    ...
    """
    # Start with detection-specific required fields
    ...
    execution_required = execution_config.get("required_fields", [])
    ...
    # Handle time window (optional based on KB)
    time_window = rule.get("time_window", {})
    if time_window.get("required", False):
        required_fields.update({"start_time", "end_time"})
    ...
    print(f"[compute_missing_fields] Required fields: {required_fields}")

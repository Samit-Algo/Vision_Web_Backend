...
from app.processing.scenarios.models import BaseScenario, ScenarioEvent, ScenarioFrameContext
from app.processing.scenarios.registry import get_scenario_class


class ScenarioEngine:
    """
    Backward-compatible name for the scenario orchestrator.

    This class manages scenario instances and coordinates per-frame processing.
    """
    ...
    def _initialize_scenarios(self) -> None:
        ...
        if initialized_count > 0:
            print(
                f"[ScenarioEngine] Initialized {initialized_count} scenario(s) for task '{self.pipeline_context.task_id}'"
            )
